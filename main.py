...
if __name__ == "__main__":
    ...
    if select==1:
        ...
        from processPosts import processRepliesJson
        ...
        topic=int(input("请输入帖子编号:"))
        from getShuiyuanPosts import getTopic
        import asyncio
        replies=asyncio.run(getTopic(topic))
        with open(f'{topic}_origin.txt',"w",encoding="utf-8") as f:
            f.write(replies)
        from processPosts import processTopic
        processTopic(topic)
        from generateWordCloud import generateWordCloud
        with open(f'{topic}_processed.txt',"r",encoding="utf-8") as f:
            content=f.read()
        generateWordCloud(content,str(topic))
