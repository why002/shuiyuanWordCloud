...
async def getTopic(topic:int)->str:
    allReplies:str=""
    page=1
    try:
        with open("cookie.txt","r",encoding="utf-8") as f:
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        while True:
            url=f"https://shuiyuan.sjtu.edu.cn/raw/{topic}?page={page}"
            try:
                async with session.get(url) as response:
                    if response.status != 200:
                        print(f"请求失败: {response.status if response else 'Network Error'}")
                ...
                print(f"获取第 {page} 页时出错: {e}")
                break
    print(f"总共获取到 {allReplies.count('-------------------------')} 条回复。")
    return allReplies

if __name__ == "__main__":
    replies=asyncio.run(getTopic(429045))
    with open("429045_origin.txt","w",encoding="utf-8") as f:
        f.write(replies)